...
const funcs = {
  login: async () => {
    ...
    console.log(
      chalk.magenta(
        figlet.textSync('AtosOne', { horizontalLayout: 'fitted' })
      )
    );
  ...
  init: async (args) => {
    ...
    appID = args[1];
    fs.writeFile('.atosone', JSON.stringify({ appID: appID }), (err) => {
      if (err) throw err;
  ...
  fspush: async (args) => {
    ...
    let appID = '';
    if (!fs.existsSync('.atosone')) {
      console.log(chalk.red(`Please run the 'init' command first.`));
      return;
    } else {
      let data = fs.readFileSync('.atosone');
    ...
    let date = new Date();
    let dest = `atosone-cli-deploy-${date.getDate()}${date.getMonth()}${date.getFullYear()}${date.getHours()}${date.getMinutes()}${date.getSeconds()}.zip`;
    let isErr = null;
