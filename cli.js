...
module.exports = {
    ...
    const questions = [
      {
        name: 'email',
        type: 'email',
        message: 'Enter your Atos-One e-mail address:',
        validate: function (value) {
          if (value.length) {
            return true;
          } else {
            return 'Please enter your Atos-One e-mail address.';
          }
        }
      },
      {
  ...
  displayHelp: () => {
    console.log(
      chalk.blue(
        figlet.textSync('Atos-One', { horizontalLayout: 'fitted' })
      )
    );
    console.log(chalk.underline('atosone-cli v1.00'));
    console.log(chalk.bold('Usage'), ': atosone-cli', chalk.yellow('command'), chalk.gray('<args>'));
